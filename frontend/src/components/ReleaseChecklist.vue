  ...
  <div class="cl-page">
    ...
    <div v-if="showNewBoard" class="overlay" @click.self="showNewBoard = false">
      <div class="modal">
        ...
        <p class="modal-desc">버전과 이름을 입력하면 기본 체크리스트 템플릿이 생성됩니다.</p>
        <div class="field">
...
export default {
  ...
  mounted() {
    ...
    if (saved) { try { this.boards = JSON.parse(saved) } catch(e) {} }
  },
  methods: {
    ...
    ringOffset(board) {
      return CIRC * (1 - this.boardProgress(board) / 100)
    },
    ...
    duplicateBoard(board) {
      ...
      copy.categories.forEach(function(cat) {
        cat.items.forEach(function(item) { item.id = makeId(); item.checked = false })
      })
...
.board-header-right { flex-shrink: 0 }
...
.check-item { display: flex; align-items: flex-start; gap: 10px; padding: 7px 10px; border-radius: 8px; cursor: pointer; transition: background .12s }
